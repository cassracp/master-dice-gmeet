/**
 * scripts/construir.js - Pipeline de Compilação Multi-Target do ReadyToRoll (R2R)
 * 
...
function criarArquivoZip(diretorioFonte, arquivoDestino) {
  return new Promise((resolvePromise, reject) => {
    ...
    const compactador = typeof archiver === 'function' 
...
function normalizarEstruturaStandalone(pastaDestino) {
  const caminhoSrcStandalone = resolve(pastaDestino, 'src/standalone/index.html');
 ...
 */
async function compilarAlvoExtensao(nomeAlvo, pastaSaida, arquivoManifestoFonte) {
  ...
  console.log(`💉 [${nomeAlvo}] Compilando Content Script Injetor (IIFE isolado)...`);
  await build({
    configFile: false,
    build: {
  ...
  if (fs.existsSync(arquivoManifestoFonte)) {
    fs.copyFileSync(arquivoManifestoFonte, resolve(pastaSaida, 'manifest.json'));
  }
...
async function compilarAlvoWeb(pastaSaida) {
  ...
  if (fs.existsSync('src/web/manifest.webmanifest')) {
    fs.copyFileSync('src/web/manifest.webmanifest', resolve(pastaSaida, 'manifest.webmanifest'));
  }
...
async function executarConstrucaoMultiTarget() {
  console.log('🚀 Iniciando pipeline de compilação multi-alvo do ReadyToRoll (R2R)...');
  ...
  await compilarAlvoExtensao('Mozilla Firefox', distFirefox, 'src/manifest.firefox.json');
  ...
  const bytesChrome = await criarArquivoZip(distChrome, resolve('dist/ready-to-roll-chrome.zip'));
  console.log(`   ✓ dist/ready-to-roll-chrome.zip (${(bytesChrome / 1024).toFixed(1)} KB)`);

  const bytesFirefox = await criarArquivoZip(distFirefox, resolve('dist/ready-to-roll-firefox.zip'));
  console.log(`   ✓ dist/ready-to-roll-firefox.zip (${(bytesFirefox / 1024).toFixed(1)} KB)`);
  ...
  console.log('   ├── dist/chrome/   -> Carregar no Chrome/Edge em "Modo do Desenvolvedor"');
  console.log('   ├── dist/firefox/  -> Carregar no Firefox (about:debugging) ou enviar .zip para AMO');
  console.log('   ├── dist/web/      -> Deploy no Vercel (vercel deploy --prod) ou GitHub Pages');
  console.log('   ├── dist/ready-to-roll-firefox.zip -> Arquivo pronto para o portal Mozilla AMO');
  console.log('   └── dist/ready-to-roll-chrome.zip  -> Arquivo pronto para a Chrome Web Store\n');
}

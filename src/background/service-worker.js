/**
 * service-worker.js - Background Service Worker do ReadyToRoll (Manifest V3)
 * 
...
async function abrirJanelaIndependente(codigoSala = '', nomeSala = '') {
  const query = codigoSala ? `?sala=${encodeURIComponent(codigoSala)}&nomeSala=${encodeURIComponent(nomeSala || '')}` : '';
  ...
  try {
    ...
    try {
      ...
      await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: () => {
          if (window.ReadyToRollInjetor && typeof window.ReadyToRollInjetor.alternarVTT === 'function') {
            window.ReadyToRollInjetor.alternarVTT();
          }
        }
      });
    } catch (erroInjecao) {
      console.warn('[ReadyToRoll SW] Injeção restrita nesta página. Abrindo como pop-up:', erroInjecao);
      await abrirJanelaIndependente();
    }
  }
});
